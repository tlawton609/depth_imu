...
SerialLineBuffer_t lpUART1LineBuffer = {lpUart1Buffer, 0, sizeof(lpUart1Buffer), printLine};
...
SerialLineBuffer_t usart1LineBuffer = {usart1Buffer, 0, sizeof(usart1Buffer), printLine};
...
SerialLineBuffer_t usart2LineBuffer = {usart2Buffer, 0, sizeof(usart2Buffer), printLine};
...
SerialLineBuffer_t usart3LineBuffer = {usart3Buffer, 0, sizeof(usart3Buffer), printLine};
...
static BaseType_t debugSerialCommand(char *writeBuffer, size_t writeBufferLen,
    ...
    if (strncmp("list", parameter, parameterStringLength) == 0) {
      ...
      if (strncmp(handle_string, "usart1", handle_string_len) == 0) {
        ...
        LL_USART_SetBaudRate((USART_TypeDef *)handle->device,
                             ...
                             LL_USART_PRESCALER_DIV1, LL_USART_OVERSAMPLING_16, new_baud);
#ifdef DEBUG_USE_LPUART1
...
void startDebugUart() {
  ...
  rval = xTaskCreate(serialGenericRxTask, "LPUartRx",
                     // TODO - verify stack size
                     configMINIMAL_STACK_SIZE, &lpuart1, DEBUG_UART_RX_TASK_PRIORITY, NULL);
  ...
  rval = xTaskCreate(serialGenericRxTask, "Usart1Rx",
                     // TODO - verify stack size
                     configMINIMAL_STACK_SIZE, &usart1, DEBUG_UART_RX_TASK_PRIORITY, NULL);
  configASSERT(rval == pdTRUE);
#endif
  ...
  rval = xTaskCreate(serialGenericRxTask, "Usart2Rx",
                     // TODO - verify stack size
                     configMINIMAL_STACK_SIZE, &usart2, DEBUG_UART_RX_TASK_PRIORITY, NULL);
  configASSERT(rval == pdTRUE);
#endif
  ...
  rval = xTaskCreate(serialGenericRxTask, "Usart3Rx",
                     // TODO - verify stack size
                     configMINIMAL_STACK_SIZE * 2, &usart3, DEBUG_UART_RX_TASK_PRIORITY, NULL);
  configASSERT(rval == pdTRUE);
#endif
...
#ifdef DEBUG_USE_LPUART1
void LPUART1_IRQHandler(void) { serialGenericUartIRQHandler(&lpuart1); }
#endif

#ifdef DEBUG_USE_USART1
void USART1_IRQHandler(void) { serialGenericUartIRQHandler(&usart1); }
#endif

#ifdef DEBUG_USE_USART2
void USART2_IRQHandler(void) { serialGenericUartIRQHandler(&usart2); }
#endif

#ifdef DEBUG_USE_USART3
void USART3_IRQHandler(void) { serialGenericUartIRQHandler(&usart3); }
#endif
